...
import { Plus, Search, Mail, Phone, MoreHorizontal, User, Edit, Trash2 } from 'lucide-react'
...
export default function TeamPage() {
  ...
  const handleSelectAll = () => {
    setSelectedMembers(
      selectedMembers.length === members.length ? [] : members.map(member => member.id)
    )
  }

  const departments = Array.from(new Set(members.map((member: TeamMember) => member.department).filter(Boolean)))
  ...
  const handleDeleteMember = async (memberId: string) => {
    ...
    try {
      ...
      // Log the activity
      if (memberToDelete) {
        await ActivityLogger.teamMemberDeleted(memberId, memberToDelete.name)
      }
